  ...
  beo.bus.on('general', function(event) {
      ...
      if (event.header == "startup") {
          ...
          if (spotifyconfig.global && spotifyconfig.global.use_mpris ) {
            spotifydsettings.mpris = true
          } else {
            spotifydsettings.mpris = false
          }
  ...
  console.log(event)
    ...
    if (event.header == "spotifydMPRIS") {
      let useMPRIS = false
      if (event.content.enabled !== undefined) {
        useMPRIS = event.content.enabled
      } else {
        return
      }

      spotifydsettings.mpris = useMPRIS;

      try {
        // Apply new login credentials
        applyChangesPreservingComments(configFile, [
          { section: "global", option: "use_mpris", value: useMPRIS },
        ]);
      } catch (error) {
